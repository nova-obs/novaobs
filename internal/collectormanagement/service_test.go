...
func TestServiceUpsertsK8sInstancesByRuntimeIdentity(t *testing.T) {
	store := memstore.NewStore()
	svc := NewService(store.CollectorGroups(), store.CollectorInstances())
	ctx := context.Background()
	...
	require.NoError(t, err)
	require.Equal(t, "opamp-uid-a", first.InstanceUID)
	require.Equal(t, "opamp-uid-a", first.OpAMPInstanceUID)
	...
	require.Equal(t, "opamp-uid-b", second.InstanceUID)
	require.Equal(t, "opamp-uid-b", second.OpAMPInstanceUID)
	require.Equal(t, "pod-uid-b", second.PodUID)
	require.Equal(t, "10.0.0.12", second.PodIP)

	instances, err := svc.ListInstances(ctx, group.ID)
	require.NoError(t, err)
	require.Len(t, instances, 1)
	require.Equal(t, "k8s:test03:"+group.ID+":node-01", instances[0].RuntimeIdentity)
