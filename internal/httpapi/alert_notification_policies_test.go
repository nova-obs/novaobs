...
func TestNotificationPolicyLifecycleAPI(t *testing.T) {
	env := newTestRouter(t)
	created := performJSON(t, env.router, http.MethodPost, "/api/v1/alerts/notification-policies", `{"name":"支付值班","receiver":"pay-oncall","enabled":true}`)
	...
	updated := performJSON(t, env.router, http.MethodPut, "/api/v1/alerts/notification-policies/"+policyID, `{"name":"支付值班（主）","receiver":"pay-oncall","enabled":true}`)
...
func TestNotificationPolicyRejectsURLAsReceiver(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/api/v1/alerts/notification-policies", bytes.NewBufferString(`{
        "name":"bad","receiver":"https://example.com/hook","enabled":true
    }`))
